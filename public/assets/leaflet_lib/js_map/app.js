...
if (rx == "Administrator" || rx == "Editor") {
  L.easyButton(
    ...
    function () {
      ...
      $(".slideshow-container_m").append(
        "<div class='m_mySlides'><img src='data/foto/no-image-icon.png' style='width:100%'></div>"
      );
      //$( "#slideshow-container_m" ).load(window.location.href + " #slideshow-container_m" );
      var tgl = new Date();
      var tglStr =
        String(tgl.getFullYear()) +
        "-" +
        String(tgl.getMonth() + 1) +
        "-" +
        String(tgl.getDate() + 1)
          .replace("Z", "")
          .replace("T", "");
      var fotoDirectoryStr =
        String(tgl.getFullYear()) +
        String(tgl.getMonth() + 1) +
        String(tgl.getDate() + 1)
          .replace("Z", "")
          .replace("T", "") +
        String(tgl.getHours()) +
        String(tgl.getMinutes()) +
        String(tgl.getSeconds()) +
        String(tgl.getMilliseconds());
      ...
      document.getElementById("formTambahSumberAirBaku").style.display =
        "block";
      currentTab = 0;
      //plusSlides(1, 0);
      //plusSlides(-1, 0);
...
map.getPane("pane_spam_pdam_unit_sentolo_ii").style["mix-blend-mode"] =
  "normal";
...
map.getPane("pane_spam_pdam_unit_sendangsari").style["mix-blend-mode"] =
  "normal";
...
map.getPane("pane_spam_pdam_unit_kalibawang").style["mix-blend-mode"] =
  "normal";
...
map.getPane("pane_spam_pdam_unit_bend_panjt_ii").style["mix-blend-mode"] =
  "normal";

map.createPane("pane_spam_pdam_unit_bend_panjt_i");
map.getPane("pane_spam_pdam_unit_bend_panjt_i").style.zIndex = 400;
map.getPane("pane_spam_pdam_unit_bend_panjt_i").style["mix-blend-mode"] =
  "normal";
...
map.getPane("pane_air_baku_sarana_pendukung_line").style["mix-blend-mode"] =
  "normal";
...
selectorKecamatan.onAdd = function (map) {
  ...
  div.innerHTML =
    '<select id="kecamatan_select" style="width: 150px;"><option value="init">Cari Kecamatan...</option></select>';
  return div;
};
  ...
  if (e.target.value == "init") {
    ...
    layer_admin_kecamatan_poly.eachLayer(function (featureInstancelayer) {
      ...
      if (propertyValue == e.target.value) {
        ...
        if (map.hasLayer(layer_admin_kabupaten_poly)) {
          map.removeLayer(layer_admin_kabupaten_poly);
        }
        if (!map.hasLayer(layer_admin_kecamatan_poly)) {
          map.addLayer(layer_admin_kecamatan_poly);
        }
        if (map.hasLayer(layer_admin_desa_poly)) {
          map.removeLayer(layer_admin_desa_poly);
        }
    ...
    if (myKecamatan.length > 0) {
      alert(
        "Ditemukan " +
          myKecamatan.length +
          " sumber air baku yang ada di Kecamatan " +
          e.target.value +
          "."
      );
    } else {
      alert(
        "Tidak ditemukan sumber air baku di Kecamatan " + e.target.value + "."
      );
    }
  }
}
...
selectorAirBaku.onAdd = function (map) {
  ...
  div.innerHTML =
    '<select id="air_baku_select" style="width: 150px;"><option value="init">Cari Sumber Air Baku...</option></select>';
  return div;
};
...
function changeHandlerAirBaku(e) {
  ...
  if (e.target.value == "init") {
    ...
    layer_air_baku_sumber_marker.eachLayer(function (layer) {
      ...
      if (
        layer.feature.properties.nama == e.target.value &&
        layer.feature.properties.kapanewon == namaKecamatan
      ) {
        layer.setIcon(pulsingIcon);
      } else {
        var myUrl = "";
        if (
          (layer.feature.properties["jenis_s"] !== null
            ? Autolinker.link(String(layer.feature.properties["jenis_s"]))
            : "") == "IPAL Komunal"
        ) {
          myUrl = "images/icons/pin-yellow.png";
        } else if (
...
var markerPopup = L.popup().setContent(
  "Bidang Cipta Karya, Dinas Pekerjaan Umum, Perumahan dan Kawasan Permukiman Kabupaten Kulon Progo"
);
...
zoomToCoordinate.onAdd = function (map) {
  ...
  div.innerHTML =
    '<input type="text" name="lat" id="inputLat" style="width: 67px;" placeholder="Latitude..." />';
  div.innerHTML +=
    '<input type="text" name="long" id="inputLong" style="width: 67px;" placeholder="Longitude..." />';
  div.innerHTML +=
    '<input type="button" id="zoomTo" onclick="zoomTo()" value="Zoom To"/>';
...
//===============================================
var layer_air_baku_sumber_marker = new L.markerClusterGroup({
  chunkedLoading: true,
});
var layer_air_baku_sarana_pendukung_marker = new L.markerClusterGroup({
  chunkedLoading: true,
});
  ...
  layer_admin_kabupaten_poly = L.geoJson(null, {
    ...
    onEachFeature: function (feature, layer) {
      if (feature.properties) {
        layer.bindTooltip(feature.properties.kabupaten.toString(), {
          noHide: true,
        });
        layer.on({
          mouseover: highlightFeature,
...
function load_GeoJSON_admin_kecamatan_poly() {
  ...
  layer_admin_kecamatan_poly = L.geoJson(null, {
    ...
    onEachFeature: function (feature, layer) {
      if (feature.properties) {
        layer.bindTooltip(feature.properties.kecamatan.toString(), {
          noHide: true,
        });
        ...
        const opt = [];
        document
          .querySelectorAll("#kecamatan_select > option")
          .forEach((option) => {
            if (opt.includes(option.value)) option.remove();
            else opt.push(option.value);
          });
      ...
      if (feature.properties) {
        var popupContent =
          ...
          (feature.properties["nama"] !== null
            ? Autolinker.link(String(feature.properties["nama"]))
            : "") +
          "</td></tr>" +
          "<tr><th>Jenis </th><td> : " +
          (feature.properties["jenis"] !== null
            ? Autolinker.link(String(feature.properties["jenis"]))
            : "") +
          "</td></tr>" +
          "<tr><th>Orde </th><td> : " +
          (feature.properties["orde"] !== null
            ? Autolinker.link(String(feature.properties["orde"]))
            : "") +
          "</td></tr>" +
          "<tr><th>Panjang </th><td> : " +
          (feature.properties["panjang"] !== null
            ? Autolinker.link(String(feature.properties["panjang"]))
            : "") +
          "</td></tr>" +
          "<tr><th>Kewenangan </th><td> : " +
          (feature.properties["kewenangan"] !== null
            ? Autolinker.link(String(feature.properties["kewenangan"]))
            : "") +
          "</td></tr>" +
          "</table>" +
          "</form>";
...
function load_GeoJSON_spam_pdam_line(panePDAM) {
  var urlPHP;
  if (panePDAM == "pane_spam_pdam_unit_bend_panjt_i") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_bend_panjt_i.php";
  } else if (panePDAM == "pane_spam_pdam_unit_bend_panjt_ii") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_bend_panjt_ii.php";
  } else if (panePDAM == "pane_spam_pdam_unit_galur") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_galur.php";
  } else if (panePDAM == "pane_spam_pdam_unit_girimulyo") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_girimulyo.php";
  } else if (panePDAM == "pane_spam_pdam_unit_kalibawang") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_kalibawang.php";
  } else if (panePDAM == "pane_spam_pdam_unit_lendah") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_lendah.php";
  } else if (panePDAM == "pane_spam_pdam_unit_sendangsari") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_sendangsari.php";
  } else if (panePDAM == "pane_spam_pdam_unit_sentolo_i") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_sentolo_i.php";
  } else if (panePDAM == "pane_spam_pdam_unit_sentolo_ii") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_sentolo_ii.php";
  } else if (panePDAM == "pane_spam_pdam_unit_sermo") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_sermo.php";
  } else if (panePDAM == "pane_spam_pdam_unit_temon_i") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_temon_i.php";
  } else if (panePDAM == "pane_spam_pdam_unit_temon_ii") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_temon_ii.php";
  } else if (panePDAM == "pane_spam_pdam_unit_wates_i") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_wates_i.php";
  } else if (panePDAM == "pane_spam_pdam_unit_wates_ii") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_wates_ii.php";
  } else if (panePDAM == "pane_spam_pdam_unit_wates_iii") {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_wates_iii.php";
  } else {
    urlPHP = "./dataservice/read_linestring_spam_pdam_unit_wates_iv.php";
  }
    ...
    onEachFeature: function (feature, layer) {
      if (feature.properties) {
        var popupContent =
          ...
          '<tr><th width="50%">Sistem </th><td> <input type="text" width="80" height="5" id="n_sistem" value="' +
          (feature.properties["sistem"] !== null
            ? Autolinker.link(String(feature.properties["sistem"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Diameter </th><td> <input type="text" width="80" height="5" id="n_diameter" value="' +
          (feature.properties["diameter"] !== null
            ? Autolinker.link(String(feature.properties["diameter"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Reservoir </th><td> <input type="text" width="80" height="5" id="n_reservoir" value="' +
          (feature.properties["reservoir"] !== null
            ? Autolinker.link(String(feature.properties["reservoir"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Existing </th><td> <input type="text" width="80" height="5" id="n_existing" value="' +
          (feature.properties["existing"] !== null
            ? Autolinker.link(String(feature.properties["existing"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Unit </th><td> <input type="text" width="80" height="5" id="n_unit" value="' +
          (feature.properties["unit"] !== null
            ? Autolinker.link(String(feature.properties["unit"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Jenis </th><td> <input type="text" width="80" height="5" id="n_jenis" value="' +
          (feature.properties["jenis"] !== null
            ? Autolinker.link(String(feature.properties["jenis"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Transdist </th><td> <input type="text" width="80" height="5" id="n_transdist" value="' +
          (feature.properties["transdist"] !== null
            ? Autolinker.link(String(feature.properties["transdist"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Panjang </th><td> <input type="text" width="80" height="5" id="n_panjang" value="' +
          (feature.properties["panjang"] !== null
            ? Autolinker.link(String(feature.properties["panjang"]))
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Keterangan </th><td> <input type="text" width="80" height="5" id="n_keterangan" value="' +
          (feature.properties["keterangan"] !== null
            ? Autolinker.link(String(feature.properties["keterangan"]))
            : "") +
          '" ></td></tr>' +
          "</table>" +
          "</form>";
        ...
        layer.bindPopup(popupContent, { minWidth: 300 });
        layer.bindTooltip(
          "Jaringan Pipa PDAM Unit " + feature.properties["unit"],
          { noHide: true }
        );
  ...
  promiseLinestring.then(function (data) {
    ...
    if (panePDAM == "pane_spam_pdam_unit_bend_panjt_i") {
      layer_spam_pdam_unit_bend_panjt_i = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_bend_panjt_ii") {
      layer_spam_pdam_unit_bend_panjt_ii = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_galur") {
      layer_spam_pdam_unit_galur = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_girimulyo") {
      layer_spam_pdam_unit_girimulyo = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_kalibawang") {
      layer_spam_pdam_unit_kalibawang = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_lendah") {
      layer_spam_pdam_unit_lendah = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_sendangsari") {
      layer_spam_pdam_unit_sendangsari = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_sentolo_i") {
      layer_spam_pdam_unit_sentolo_i = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_sentolo_ii") {
      layer_spam_pdam_unit_sentolo_ii = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_sermo") {
      layer_spam_pdam_unit_sermo = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_temon_i") {
      layer_spam_pdam_unit_temon_i = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_temon_ii") {
      layer_spam_pdam_unit_temon_ii = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_wates_i") {
      layer_spam_pdam_unit_wates_i = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_wates_ii") {
      layer_spam_pdam_unit_wates_ii = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_wates_iii") {
      layer_spam_pdam_unit_wates_iii = layerPDAM;
    } //.addTo(map);}
    if (panePDAM == "pane_spam_pdam_unit_wates_iv") {
      layer_spam_pdam_unit_wates_iv = layerPDAM;
    } //.addTo(map);}
  });
}
    ...
    onEachFeature: function (feature, layer) {
      if (feature.properties) {
        var popupContent =
          ...
          '<table width="296">' +
          '<tr><th width="50%">ID </th><td> <input type="text" width="80" height="5" id="n_gid" value="' +
          (feature.properties["gid"] !== null
            ? Autolinker.link(String(feature.properties["gid"]))
            : "") +
          '" readonly></td></tr>' +
          '<tr><th width="50%">Nama </th><td> <input type="text" width="80" height="5" id="n_nama" nama="' +
          (feature.properties["nama"] !== null
            ? Autolinker.link(String(feature.properties["nama"]))
            : "") +
          '" ></td></tr>' +
          ...
          (feature.properties["keterangan"] !== null
            ...
            : "") +
          '" ></td></tr>' +
          '<tr><th width="50%">Kondisi </th><td> <input type="text" width="80" height="5" id="n_kondisi" value="' +
          (feature.properties["kondisi"] !== null
            ? Autolinker.link(String(feature.properties["kondisi"]))
            : "") +
          '" ></td></tr>' +
          "</table>" +
          "</form>";
...
function load_GeoJSON_air_baku_sarana_pendukung_point() {
    ...
    pointToLayer: function (feature, latlng) {
      ...
      var popupContent =
        ...
        (feature.properties["gid"] !== null
          ...
          : "") +
        '" readonly></td></tr>' +
        '<tr><th width="50%">Nama </th><td> <input type="text" width="80" height="5" id="n_nama" value="' +
        (feature.properties["nama"] !== null
          ? Autolinker.link(String(feature.properties["nama"]))
          : "") +
        '" ></td></tr>' +
      ...
      myMarker.bindPopup(popupContent, { minWidth: 300 });
      return myMarker;
    },
    // onEachFeature: function (feature, layer) {
    // 	if (feature.properties) {
  ...
  promisePoint.then(function (data) {
    layer_air_baku_sarana_pendukung_point
      .addData(data)
      .addTo(layer_air_baku_sarana_pendukung_marker.addTo(map));
    // map.addLayer(layer_air_baku_sarana_pendukung_point);
  });
}

function TambahPhoto(
  strImgManager,
  strLoadImage,
  strDirectory,
  strPhotoDirectory,
  strSlideshowContainer,
  strSlides,
  idxSlides
) {
  ...
  myInput.addEventListener(
    ...
    function (evt) {
      ...
      if (myImg.length > 0) {
        ...
        for (var i = 0; i < myImg.length; i++) {
          if (myImg[i].size > 1048576) {
            ...
            alert(
              "Mohon maaf, '" +
                myImg[i] +
                "' gagal diunggah ke dalam database. Ukuran file harus di bawah " +
                1048576 / 1024 / 1024 +
                " MB!"
            );
          } else {
            ...
            var json_photo = JSON.stringify(
              document.getElementById(strPhotoDirectory).value
            );
            ...
            xhttp.open(
              "POST",
              "/airbersihku/gis/dataservice/upload_file.php",
              true
            );
                ...
                for (var ii = 0; ii < myImg.length; ii++) {
                  if (myImg[ii].size > 1048576) {
                    ...
                    var img_slideshow =
                      ...
                      document.getElementById(strPhotoDirectory).value +
                      "/" +
                      String(myImg[ii].name);
                    if (!myImgUpdated.includes(img_slideshow)) {
                      $(String("." + strSlideshowContainer)).append(
                        "<div class='" +
                          strSlides +
                          "'><img src='" +
                          img_slideshow +
        ...
        $("#" + strLoadImage).remove();
        $("#" + strImgManager).append(
          "<input id='" +
            strLoadImage +
            "' style='visibility:hidden;' type='file' multiple='multiple' accept='.jpg, .jpeg, .png, .gif, .tif, .tiff, .bmp|image/*' >"
        );
      } else {
  ...
  for (var i = 0; i < x.length; i++) {
    if (x[i].style.display == "block") {
      ...
      var deleteConfirmation = confirm(
        "Apakah Anda yakin akan menghapus foto ini dari database? Data akan terhapus dari database dan tidak dapat dikembalikan!"
      );
            ...
            if (response == 1) {
              ...
              if (y.length == 0) {
                $(String("." + strSlideshowContainer)).append(
                  "<div class='" +
                    strSlides +
                    "'><img src='data/foto/no-image-icon.png' style='width:100%'></div>"
                );
                plusSlides(1, idxSlides);
              }
            } else {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.response == "200") {
        ...
        div_formTambahSumberAirBakuHide();
        var reloadConfirmation = confirm(
          "Data berhasil disimpan dalam database. Muat ulang (reload) halaman WebGIS untuk sinkronisasi data dan melihat hasilnya?"
        );
        ...
        alert(
          "Mohon maaf, data gagal disimpan dalam database.. Error:" +
            data.message +
            "."
        );
      }
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.response == "200") {
        ...
        currentTab = 0;
        var reloadConfirmation = confirm(
          "Pembaruan data berhasil disimpan dalam database. Muat ulang (reload) halaman WebGIS untuk sinkronisasi data dan melihat hasilnya?"
        );
        if (reloadConfirmation == true) {
          window.location.reload();
        }
        ...
        alert(
          "Mohon maaf, pembaruan data gagal disimpan dalam database.. Error:" +
            data.message +
            "."
        );
      }
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data.response == "200") {
        ...
        currentTab = 0;
        var reloadConfirmation = confirm(
          "Terima kasih atas partisipasi yang diberikan. Laporan pengaduan Anda telah disampaikan kepada Administrator untuk dapat ditindak-lanjuti. "
        );
        if (reloadConfirmation == true) {
          window.location.reload();
        }
        ...
        console.log("Failed to update.");
        alert(
          "Mohon maaf, laporan pengaduan gagal dikirim.. Error:" +
            data.message +
            "."
        );
      }
...
function HapusSumberAirBaku() {
  var gid = document.getElementById("n_gid").value;
  var deleteConfirmation = confirm(
    "Apakah Anda yakin akan menghapus data ini? Data akan terhapus dari database dan tidak dapat dikembalikan!"
  );
  if (deleteConfirmation == true) {
    $.ajax({
      ...
      success: function (data) {
        if (data.response == "200") {
          ...
          currentTab = 0;
          var reloadConfirmation = confirm(
            "Data berhasil dihapus dari database. Muat ulang (reload) halaman WebGIS untuk sinkronisasi data dan melihat hasilnya?"
          );
          if (reloadConfirmation == true) {
            window.location.reload();
          }
          ...
          alert(
            "Mohon maaf, data gagal dihapus dari database.. Error:" +
              data.message +
              "."
          );
        }
...
function load_GeoJSON_air_baku_sumber_point() {
  ...
  layer_air_baku_sumber_point = L.geoJson(null, {
    ...
    pointToLayer: function (feature, latlng) {
      ...
      var myUrl = "";
      if (
        (feature.properties["jenis_s"] !== null
          ? Autolinker.link(String(feature.properties["jenis_s"]))
          : "") == "Mata Air"
      ) {
      ...
      if (
        (feature.properties["status_p"] !== null
          ? Autolinker.link(String(feature.properties["status_p"]))
          : "") == "Dilaporkan"
      ) {
      ...
      const folder =
        ...
        (feature.properties["foto"] !== null
          ? Autolinker.link(String(feature.properties["foto"]))
          : "") +
        "/";
      var mySS = "";
      ...
      $.ajax({
        ...
        complete: function () {
          if (mySS == "") {
            mySS =
              '<div class="n_mySlides"><img src="data/foto/no-image-icon.png" style="width:100%"></div>';
          }
          ...
          if (rx == "Administrator" || rx == "Editor") {
            popupContent +=
              ...
              '<button type="button" id="addPhotoBtn_n" onclick="TambahPhoto(\'' +
              strImgManager_n +
              "','" +
              strLoadImage_n +
              "','" +
              ...
              n_foto +
              "','" +
              strSlideshowContainer_n +
              "','" +
              slideId[1] +
              "','" +
              slideIndex[1] +
              '\')"> + </button> <input id="loadImage_n" style="visibility:hidden;" type="file" multiple="multiple" accept=".jpg, .jpeg, .png, .gif, .tif, .tiff, .bmp" >' +
              "</div>" +
              '<div style="float:right;">' +
              '<button type="button" id="removePhotoBtn_n" onclick="HapusPhoto(\'' +
              strSlideshowContainer_n +
              "','" +
              slideId[1] +
              "','" +
              ...
              '<tr><th width="50%">Nomor ID</th><td> <input  [type="text"] style="width: 150px;" id="n_gid" value="' +
              (feature.properties["gid"] !== null
                ? Autolinker.link(String(feature.properties["gid"]))
                : "") +
              '" readonly></td></tr>' +
              '<tr><th width="50%">Nama Sumber Air Baku </th><td> <input  [type="text"] style="width: 150px;" id="n_nama" value="' +
              (feature.properties["nama"] !== null
                ? Autolinker.link(String(feature.properties["nama"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Latitude </th><td> <input  [type="text"] style="width: 150px;" id="n_latitude" value="' +
              (feature.properties["latitude"] !== null
                ? Autolinker.link(String(feature.properties["latitude"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Longitude </th><td> <input  [type="text"] style="width: 150px;" id="n_longitude" value="' +
              (feature.properties["longitude"] !== null
                ? Autolinker.link(String(feature.properties["longitude"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Altitude </th><td> <input  [type="text"] style="width: 150px;" id="n_altitude" value="' +
              (feature.properties["altitude"] !== null
                ? Autolinker.link(String(feature.properties["altitude"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Dusun </th><td> <input  [type="text"] style="width: 150px;" id="n_dusun" value="' +
              (feature.properties["dusun"] !== null
                ? Autolinker.link(String(feature.properties["dusun"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Kalurahan </th><td> <input  [type="text"] style="width: 150px;" id="n_kalurahan" value="' +
              (feature.properties["kalurahan"] !== null
                ? Autolinker.link(String(feature.properties["kalurahan"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Kapanewon </th><td> <select style="width: 150px; height: 21px;" id="n_kapanewon"><option value="' +
              (feature.properties["kapanewon"] !== null
                ? Autolinker.link(String(feature.properties["kapanewon"]))
                : "") +
              '" selected disabled hidden>' +
              (feature.properties["kapanewon"] !== null
                ? Autolinker.link(String(feature.properties["kapanewon"]))
                : "") +
              '</option><option value="Samigaluh">Samigaluh</option><option value="Wates">Wates</option><option value="Pengasih">Pengasih</option><option value="Kalibawang">Kalibawang</option><option value="Girimulyo">Girimulyo</option><option value="Panjatan">Panjatan</option><option value="Temon">Temon</option><option value="Nanggulan">Nanggulan</option><option value="Lendah">Lendah</option><option value="Sentolo">Sentolo</option><option value="Galur">Galur</option><option value="Kokap">Kokap</option></select></td></tr>' +
              "</table>" +
              "</div>" +
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Jenis Sumber Air </th><td> <select style="width: 150px; height: 21px;" id="n_jenis_s"><option value="' +
              (feature.properties["jenis_s"] !== null
                ? Autolinker.link(String(feature.properties["jenis_s"]))
                : "") +
              '" selected disabled hidden>' +
              (feature.properties["jenis_s"] !== null
                ? Autolinker.link(String(feature.properties["jenis_s"]))
                : "") +
              '</option><option value="Mata Air">Mata Air</option><option value="Sumur Bor">Sumur Bor</option><option value="Sumur Gali">Sumur Gali</option><option value="Air Permukaan">Air Permukaan</option><option value="Penampungan Air Hujan">Penampungan Air Hujan</option></select></td></tr>' +
              '<tr><th width="50%">Sistem </th><td> <input  [type="text"] style="width: 150px;" id="n_sistem" value="' +
              (feature.properties["sistem"] !== null
                ? Autolinker.link(String(feature.properties["sistem"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Kpsts Produksi (l/det) </th><td> <input  [type="text"] style="width: 150px;" id="n_kpsts_p" value="' +
              (feature.properties["kpsts_p"] !== null
                ? Autolinker.link(String(feature.properties["kpsts_p"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Kpsts Terpasang (l/det) </th><td> <input  [type="text"] style="width: 150px;" id="n_kpsts_t" value="' +
              (feature.properties["kpsts_t"] !== null
                ? Autolinker.link(String(feature.properties["kpsts_t"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Data Idle </th><td> <input  [type="text"] style="width: 150px;" id="n_data_idle" value="' +
              (feature.properties["data_idle"] !== null
                ? Autolinker.link(String(feature.properties["data_idle"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jenis Pipa </th><td> <input  [type="text"] style="width: 150px;" id="n_p_jenis" value="' +
              (feature.properties["p_jenis"] !== null
                ? Autolinker.link(String(feature.properties["p_jenis"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Panjang Pipa </th><td> <input  [type="text"] style="width: 150px;" id="n_p_panjang" value="' +
              (feature.properties["p_panjang"] !== null
                ? Autolinker.link(String(feature.properties["p_panjang"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Diameter Pipa </th><td> <input  [type="text"] style="width: 150px;" id="n_p_diameter" value="' +
              (feature.properties["p_diameter"] !== null
                ? Autolinker.link(String(feature.properties["p_diameter"]))
                : "") +
              '" ></td></tr>' +
              "</table>" +
              "</div>" +
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Daerah Layanan </th><td> <input  [type="text"] style="width: 150px;" id="n_layanan" value="' +
              (feature.properties["layanan"] !== null
                ? Autolinker.link(String(feature.properties["layanan"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jumlah Infrastruktur SR </th><td> <input  [type="text"] style="width: 150px;" id="n_sr" value="' +
              (feature.properties["sr"] !== null
                ? Autolinker.link(String(feature.properties["sr"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jumlah Infrastruktur HU </th><td> <input  [type="text"] style="width: 150px;" id="n_hu" value="' +
              (feature.properties["hu"] !== null
                ? Autolinker.link(String(feature.properties["hu"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jumlah Penduduk (KK) </th><td> <input  [type="text"] style="width: 150px;" id="n_pddk_kk" value="' +
              (feature.properties["pddk_kk"] !== null
                ? Autolinker.link(String(feature.properties["pddk_kk"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jumlah Penduduk (Jiwa) </th><td> <input  [type="text"] style="width: 150px;" id="n_pddk_jiwa" value="' +
              (feature.properties["pddk_jiwa"] !== null
                ? Autolinker.link(String(feature.properties["pddk_jiwa"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jumlah Pelanggan (KK) </th><td> <input  [type="text"] style="width: 150px;" id="n_plgn_kk" value="' +
              (feature.properties["plgn_kk"] !== null
                ? Autolinker.link(String(feature.properties["plgn_kk"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Jumlah Pelanggan (Jiwa) </th><td> <input  [type="text"] style="width: 150px;" id="n_plgn_jiwa" value="' +
              (feature.properties["plgn_jiwa"] !== null
                ? Autolinker.link(String(feature.properties["plgn_jiwa"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Tarif Air (Rp/m3) </th><td> <input  [type="text"] style="width: 150px;" id="n_tarif" value="' +
              (feature.properties["tarif"] !== null
                ? Autolinker.link(String(feature.properties["tarif"]))
                : "") +
              '" ></td></tr>' +
              "</table>" +
              "</div>" +
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Tahun Pembangunan </th><td> <input  [type="text"] style="width: 150px;" id="n_t_dibangun" value="' +
              (feature.properties["t_dibangun"] !== null
                ? Autolinker.link(String(feature.properties["t_dibangun"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Tahun Renovasi </th><td> <input  [type="text"] style="width: 150px;" id="n_t_direnov" value="' +
              (feature.properties["t_direnov"] !== null
                ? Autolinker.link(String(feature.properties["t_direnov"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Sumber Dana </th><td> <input  [type="text"] style="width: 150px;" id="n_sbr_dana" value="' +
              (feature.properties["sbr_dana"] !== null
                ? Autolinker.link(String(feature.properties["sbr_dana"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Tahun Anggaran </th><td> <input  [type="text"] style="width: 150px;" id="n_t_anggaran" value="' +
              (feature.properties["t_anggaran"] !== null
                ? Autolinker.link(String(feature.properties["t_anggaran"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Dana APBN </th><td> <select style="width: 150px; height: 21px;" id="n_apbn"><option value="' +
              (feature.properties["apbn"] !== null
                ? Autolinker.link(String(feature.properties["apbn"]))
                : "") +
              '" selected disabled hidden>' +
              (feature.properties["apbn"] !== null
                ? Autolinker.link(String(feature.properties["apbn"]))
                : "") +
              '</option><option value="Ya">Ya</option><option value="Tidak">Tidak</option></select></td></tr>' +
              '<tr><th width="50%">Dana APBD </th><td> <select style="width: 150px; height: 21px;" id="n_apbd"><option value="' +
              (feature.properties["apbd"] !== null
                ? Autolinker.link(String(feature.properties["apbd"]))
                : "") +
              '" selected disabled hidden>' +
              (feature.properties["apbd"] !== null
                ? Autolinker.link(String(feature.properties["apbd"]))
                : "") +
              '</option><option value="Ya">Ya</option><option value="Tidak">Tidak</option></select></td></tr>' +
              '<tr><th width="50%">Ketua Pengelola </th><td> <input  [type="text"] style="width: 150px;" id="n_pengelola" value="' +
              (feature.properties["pengelola"] !== null
                ? Autolinker.link(String(feature.properties["pengelola"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Contact Person </th><td> <input  [type="text"] style="width: 150px;" id="n_cp" value="' +
              (feature.properties["cp"] !== null
                ? Autolinker.link(String(feature.properties["cp"]))
                : "") +
              '" ></td></tr>' +
              "</table>" +
              "</div>" +
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Keterangan </th><td> <textarea name="Text1" style="width: 150px;" id="n_keterangan" >' +
              (feature.properties["keterangan"] !== null
                ? Autolinker.link(String(feature.properties["keterangan"]))
                : "") +
              "</textarea></td></tr>" +
              '<tr><th width="50%">Folder Foto </th><td> <input  [type="text"] style="width: 150px;" id="n_foto" value="' +
              (feature.properties["foto"] !== null
                ? Autolinker.link(String(feature.properties["foto"]))
                : "") +
              '" ></td></tr>' +
              ...
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Laporan </th><td> <textarea name="Text1" style="width: 150px;" id="n_laporan" >' +
              (feature.properties["laporan"] !== null
                ? Autolinker.link(String(feature.properties["laporan"]))
                : "") +
              "</textarea></td></tr>" +
              '<tr><th width="50%">Tanggal Laporan </th><td> <input type="date" placeholder="dd-mm-yyyy" style="width: 150px;" id="n_tanggal_l" value="' +
              (feature.properties["tanggal_l"] !== null
                ? Autolinker.link(String(feature.properties["tanggal_l"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Pelapor </th><td> <input  [type="text"] style="width: 150px;" id="n_pelapor" value="' +
              (feature.properties["pelapor"] !== null
                ? Autolinker.link(String(feature.properties["pelapor"]))
                : "") +
              '" ></td></tr>' +
              "</table>" +
              "</div>" +
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Penanganan </th><td> <textarea name="Text2" style="width: 150px;" id="n_penanganan" >' +
              (feature.properties["penanganan"] !== null
                ? Autolinker.link(String(feature.properties["penanganan"]))
                : "") +
              "</textarea></td></tr>" +
              '<tr><th width="50%">Tanggal Penanganan </th><td> <input type="date" placeholder="yyyy-mm-dd" style="width: 150px;" id="n_tanggal_p" value="' +
              (feature.properties["tanggal_p"] !== null
                ? Autolinker.link(String(feature.properties["tanggal_p"]))
                : "") +
              '" ></td></tr>' +
              '<tr><th width="50%">Status </th><td> <select style="width: 150px; height: 21px;" id="n_status_p"><option value="' +
              (feature.properties["status_p"] !== null
                ? Autolinker.link(String(feature.properties["status_p"]))
                : "") +
              '" selected disabled hidden>' +
              (feature.properties["status_p"] !== null
                ? Autolinker.link(String(feature.properties["status_p"]))
                : "") +
              '</option><option value="Dilaporkan">Dilaporkan</option><option value="Sedang Ditinjau">Sedang Ditinjau</option><option value="Proses Penanganan">Proses Penanganan</option><option value="Selesai Penanganan">Selesai Penanganan</option></select></td></tr>' +
              '<tr><th width="50%">Operator </th><td> <input  [type="text"] style="width: 150px;" id="n_operator" value="' +
              (feature.properties["operator"] !== null
                ? Autolinker.link(String(feature.properties["operator"]))
                : "") +
              '" ></td></tr>' +
              "</table>" +
              "</div>" +
              '<div style="text-align:center;margin-top:5px;">' +
              '<button type="button" id="n_prevBtn" onclick="nextPrev(-1, 7, \'' +
              tab_n +
              "','" +
              step_n +
              "','" +
              ...
              '<span class="step n"></span>' +
              '<span class="step n"></span>' +
              '<button type="button" id="n_nextBtn" onclick="nextPrev(1, 7, \'' +
              tab_n +
              "','" +
              step_n +
              "','" +
            ...
            popupContent +=
              ...
              (feature.properties["kalurahan"] !== null
                ...
                : "") +
              '" readonly></td></tr>' +
              '<tr><th width="50%">Kapanewon </th><td> <input  [type="text"] style="width: 150px;" id="n_kapanewon" value="' +
              (feature.properties["kapanewon"] !== null
                ? Autolinker.link(String(feature.properties["kapanewon"]))
                : "") +
              '" readonly></td></tr>' +
              ...
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Jenis Sumber Air </th><td> <input  [type="text"] style="width: 150px;" id="n_jenis_s" value="' +
              (feature.properties["jenis_s"] !== null
                ? Autolinker.link(String(feature.properties["jenis_s"]))
                : "") +
              '" readonly></td></tr>' +
              ...
              (feature.properties["t_anggaran"] !== null
                ...
                : "") +
              '" readonly></td></tr>' +
              '<tr><th width="50%">Dana APBN </th><td> <input  [type="text"] style="width: 150px;" id="n_apbn" value="' +
              (feature.properties["apbn"] !== null
                ? Autolinker.link(String(feature.properties["apbn"]))
                : "") +
              '" readonly></td></tr>' +
              '<tr><th width="50%">Dana APBD </th><td> <input  [type="text"] style="width: 150px;" id="n_apbd" value="' +
              (feature.properties["apbd"] !== null
                ? Autolinker.link(String(feature.properties["apbd"]))
                : "") +
              '" readonly></td></tr>' +
              ...
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Keterangan </th><td> <textarea name="Text1" style="width: 150px;" id="n_keterangan" readonly>' +
              (feature.properties["keterangan"] !== null
                ? Autolinker.link(String(feature.properties["keterangan"]))
                : "") +
              "</textarea></td></tr>" +
              ...
              (feature.properties["laporan"] !== null
                ...
                : "") +
              "</textarea></td></tr>" +
              '<tr><th width="50%">Tanggal Laporan </th><td> <input  [type="text"] style="width: 150px;" id="n_tanggal_l" value="' +
              (feature.properties["tanggal_l"] !== null
                ? Autolinker.link(String(feature.properties["tanggal_l"]))
                : "") +
              '" readonly></td></tr>' +
              ...
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Penanganan </th><td> <textarea name="Text2" style="width: 150px;" id="n_penanganan" readonly>' +
              (feature.properties["penanganan"] !== null
                ? Autolinker.link(String(feature.properties["penanganan"]))
                : "") +
              "</textarea></td></tr>" +
              '<tr><th width="50%">Tanggal Penanganan </th><td> <input  [type="text"] style="width: 150px;" id="n_tanggal_p" value="' +
              (feature.properties["tanggal_p"] !== null
                ? Autolinker.link(String(feature.properties["tanggal_p"]))
                : "") +
              '" readonly></td></tr>' +
              '<tr><th width="50%">Status </th><td> <input  [type="text"] style="width: 150px;" id="n_status_p" value="' +
              (feature.properties["status_p"] !== null
                ? Autolinker.link(String(feature.properties["status_p"]))
                : "") +
              '" readonly></td></tr>' +
            ...
            popupContent +=
              ...
              '<div class="tab n">' +
              '<table border="0" width="296">' +
              '<tr><th width="50%">Laporan </th><td> <textarea name="Text1" style="width: 150px;" id="n_laporan" readonly>' +
              (feature.properties["laporan"] !== null
                ? Autolinker.link(String(feature.properties["laporan"]))
                : "") +
              "</textarea></td></tr>" +
  ...
  promisePoint.then(function (data) {
    layer_air_baku_sumber_point
      .addData(data)
      .addTo(layer_air_baku_sumber_marker.addTo(map));
...
var miniMap = new L.Control.MiniMap(basemapLayersCopy.layer_OSM, {
  width: 150,
  height: 150,
  toggleDisplay: true,
